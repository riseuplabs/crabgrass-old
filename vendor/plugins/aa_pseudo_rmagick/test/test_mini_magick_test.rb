...
class ImageTest < Test::Unit::TestCase
  ...
  SIMPLE_IMAGE_PATH = CURRENT_DIR + "frog.gif"
  TIFF_IMAGE_PATH = CURRENT_DIR + "frog.tiff"
  ...
  def test_not_an_image
    assert_raise(ImageMagickError) do
  ...
  def test_image_meta_info
    image = Image.new(SIMPLE_IMAGE_PATH)
    assert_equal 107, image[:width]
    assert_equal 77, image[:height]
