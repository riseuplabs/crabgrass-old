...
class PeopleController < ApplicationController
  ...
  def index
    @users = User.recent.paginate :page => @page_number if logged_in?
  end

  def contacts
    return unless logged_in?

    @users = (User.contacts_of(current_user).alphabetized(@letter_page)).paginate :page => @page_number
  ...
  def peers
    ...
    @users = User.peers_of(current_user).alphabetized(@letter_page).paginate :page => @page_number
     # what letters can be used for pagination
  ...
  def directory
    ...
    @users = User.alphabetized(@letter_page).paginate :page => @page_number
    # what letters can be used for pagination
