module Admin::PostsHelper
  ...
  def post_link(post)
    if post.discussion.page
      link_to_if(post.deleted_at.nil?, post.body_html[0..60] + '...', "#{page_url(post.discussion.page)}#posts-#{post.id}")
    elsif post.discussion.commentable
      link_to_if(post.deleted_at.nil?, post.body_html[0..60] + '...', url_for_user(post.discussion.commentable))
    end
  end
