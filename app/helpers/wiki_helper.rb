module WikiHelper
  ...
  def old_version_select_tag(wiki, spinner_id)
    ...
    select_tag select_tag_name, select_tag_options,
      :onchange => (remote_function(:url => wiki_action('old_version', :wiki_id => wiki.id),
                                      ...
                                      :confirm => "Any unsaved text will be lost. Are you sure?"[:wiki_lost_text_confirmation]))
  end
  ...
  def wiki_locked_notice(wiki)
    ...
    error_text = "This wiki is currently locked by {user}"[:wiki_is_locked, {:user => wiki.locker_of(:document).try.name || 'unkown'[:unknown]}]
