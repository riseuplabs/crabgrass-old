module GroupsHelper
  ...
  def destroy_group_link
    ...
    if may_destroy_group?
      link_to_with_confirm("Destroy {group_type}"[:destroy_group_link, @group.group_type],
                        {:confirm => "Are you sure you want to delete this {thing}? This action cannot be undone."[:destroy_confirmation, @group.group_type.downcase], :url => groups_url(:action => :destroy), :method => :post})
    end
  end
  ...
  def edit_featured_link(label=nil)
    label ||= "edit featured content"[:edit_featured_content_link].titlecase
  ...
  def edit_group_custom_appearance_link(appearance)
    if appearance and may_edit_appearance?
      link_to "edit custom appearance"[:edit_appearance], edit_custom_appearance_url(appearance)
    end
  end
