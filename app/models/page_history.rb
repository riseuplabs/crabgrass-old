class PageHistory < ActiveRecord::Base
  ...
  def self.pending_digest_notifications_by_page
    histories = {}
    PageHistory.find(:all, :order => "created_at desc", :conditions => ['notification_digest_sent_at IS NULL and created_at <= ?', 2.days.ago]).each do |page_history|
  ...
  def self.pending_notifications
    PageHistory.find :all, :conditions => ['notification_sent_at IS NULL and created_at <= ?', 2.days.ago]
  end
