#
# An inappropriate flag request 
#
# creator: user flagging a page, post, etc as inappopriate 
# recipient: moderation group (for now) 
# requestable: nil
#
#
module RequestToFlagInnapropriate

#  validates_format_of :recipient_type, :with => /User/
#
#  def validate_on_create
#    if Friendship.find_by_user_id_and_contact_id(created_by_id, recipient_id)
#      errors.add_to_base('Friendship already exists')
#    end
#    if RequestToFriend.appearing_as_state(state).find_by_created_by_id_and_recipient_id(created_by_id, recipient_id)
#      errors.add_to_base('Request already exists for %s'[:request_exists_error] % recipient.name)
#    end
#  end
#
#  def requestable_required?() false end
###
# are we allowed to make the request?
###
#
  def may_create?(user)
    true
  end

  def may_destroy?(user)
    user == recipient or user == created_by
  end

  def may_approve?(user)
    recipient == user
  end

  def may_view?(user)
    user == recipient or may_approve?(user)
  end

  def after_approval
    # 1. delete page
  end

  def description
    ":user would like to be the friend of :other_user"[:request_to_friend_description] % {
       :user => user_span(created_by), :other_user => user_span(recipient)
    }
  end

end
