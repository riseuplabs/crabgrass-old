  ...
  def public_line
    if current_user.may?(:admin, @page)
      ...
      if !@page.public_requested?
        checkbox_line = sidebar_checkbox('make public'[:page_make_public_link], checked, url, li_id, checkbox_id, :title => "If checked, anyone may view this page."[:public_checkbox_help])
      elsif !@page.public?
        checkbox_line = sidebar_checkbox('make public (pending)'[:page_make_public_pending_link], checked, url, li_id, checkbox_id, :title => "If checked, anyone may view this page."[:public_checkbox_help])
      end
