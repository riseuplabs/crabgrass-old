  ...
  def public_line
    if current_user.may?(:admin, @page)
      ...
      if !@page.public_requested?
        checkbox_line = sidebar_checkbox('make public'.t, checked, url, li_id, checkbox_id)
      elsif !@page.public?
        checkbox_line = sidebar_checkbox('make public (pending)'.t, checked, url, li_id, checkbox_id)
      end
