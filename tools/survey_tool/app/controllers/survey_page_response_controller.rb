...
class SurveyPageResponseController < BasePageController
  ...
  def make
    ...
    flash_message :success => "Thank you for submitting your response."[:survey_thanks_submit_message]
    if may_rate_survey_response?
      flash_message :success => "Please check your answers and rate other peoples responses."[:survey_please_check_and_rate_message]
    else
      flash_message :success => "Please check your answers."[:survey_please_check_message]
    end
